...
*/
//Derivative From tapenade
//By-hand alteration to use M_PI constant
...
//Copy of derivative generated by clad. 
//Modified by hand to remove the tape usage.
void auto_gauss_grad_0_1_2(double *x, double *p, double sigma, int dim, clad::array_ref<double> _d_x, clad::array_ref<double> _d_p, clad::array_ref<double> _d_sigma) {
  int _d_dim = 0;
  double _d_t = 0;
  unsigned long _t0;
  int _d_i = 0;
 ...
 _label0:
  ...
  for (; _t0; _t0--) {
    double _r_d0 = _d_t;
    _d_t += _r_d0;
    double _r0 = _r_d0 * (x[_t0] - p[_t0] );
    _d_x[_t0] += _r0;
    _d_p[_t0] += -_r0;
    double _r1 = (x[_t0] - p[_t0] ) * _r_d0;
    _d_x[_t0] += _r1;
    _d_p[_t0] += -_r1;
    _d_t -= _r_d0;
  }
...
//tapenade functions
typedef void(*func_tap) (double* x, double *xb, double* p, double *pb, double sigma, 
			 double *sigmab, int dim, double *retval, double *retvalb); 
...
void compute_clad( func op, double* d_x, double* d_y,
...
void compute_tap(func_tap op, double* d_x, double* d_y,
		 double* d_sigma, int i, int dim, double* result_dx,
		 double* result_dy) {
...
int main(int argc, char *argv[]) {
  ...
  for ( unsigned int i=0; i<N; i++)
    compute_clad(gauss_grad_0_1_2, x, y, sigma, i, dim, result_x, result_y);
  ...
  start = std::chrono::high_resolution_clock::now();
   // The computation kernel is launched by the statement:
  for ( unsigned int i=0; i<N; i++)
    compute_tap(gauss_d_tap, x, y, sigma, i, dim, result_x, result_y);
  stop = std::chrono::high_resolution_clock::now();
  ...
  start = std::chrono::high_resolution_clock::now();
   // The computation kernel is launched by the statement:
  for ( unsigned int i=0; i<N; i++)
    compute_clad( auto_gauss_grad_0_1_2, x, y, sigma, i, dim, result_x, result_y);
  stop = std::chrono::high_resolution_clock::now();
  auto duration_auto_clad = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  ...
  std::cout << "Hand altered Clad time " <<duration_auto_clad.count() << " microsec " << (duration_auto_clad.count()/float(N)) << " per call to compute" <<std::endl;
